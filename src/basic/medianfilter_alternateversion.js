...
const medianFilter_alternateversion = (kernel) => (image,copy=true) => {
    ...
    let DiameterKernel = DiameternoCenter+1;
    const flatten = (array) => {
	...
	outputRaster.pixelData = Array(image.height).fill(0).reduce((acc4, element4, index4) => {
	    let histogramList = Array(image.width+DiameternoCenter).fill(0).reduce((acc,element) => { return acc.concat([Array(DiameterKernel).fill(0)]); },[]);
	    paddedImage.slice(index4*(image.width+DiameternoCenter),(index4+DiameterKernel)*(image.width+(radiusKernel)*2))
		.forEach((element2, index2) => {
		    histogramList[index2%(image.width+DiameternoCenter)][Math.floor(index2/(image.width+DiameternoCenter))] = element2;
		});
	    let rowMedian = histogramList.slice(0,image.width)
		.reduce((acc, element, index) => {
		    let kernelHistogram = histogramList.slice(index,index+DiameterKernel)
	...
	let histSize = 256;
	let histogramList = Array(image.width+DiameternoCenter).fill(0).reduce((acc,element) => { return acc.concat([Array(histSize).fill(0)]); },[]);
	paddedImage.filter((element, index) => {
	    return index < (DiameterKernel)*(image.width+DiameternoCenter);
	}).forEach((element2, index2) => {
	    histogramList[index2%(image.width+DiameternoCenter)][element2] = histogramList[index2%(image.width+DiameternoCenter)][element2]+1;
	});
	...
	Array((image.height+(radiusKernel)*2)-radiusKernel*2-1).fill(0).reduce((acc,element,index) => {
		...
		newKernelHistogram[paddedImage[(image.width+DiameternoCenter)*index+index2]]--;
		newKernelHistogram[paddedImage[(image.width+DiameternoCenter)*(index+DiameterKernel)+index2]]++;
	...
	Array(Math.floor((image.width+DiameternoCenter)-DiameternoCenter-1)).fill(0).forEach((element, index) => {	
	...
	let halfNbPixelKernel = ((DiameterKernel)*(DiameterKernel)-1)/2;
	...
	ltmedian = Array(histSize).fill(0).reduce((acc, element, index) => {
	    ...
	    return (histogramList[columnIndex+DiameterKernel][index] != 0 && index < median) ? acc+histogramList[columnIndex+DiameterKernel][index] : acc;	    
	}, ltmedian);
	while (ltmedian > halfNbPixelKernel){
	    median--;
	    let sumMedian = 0;
	    Array(DiameterKernel).fill(0).forEach((element, index) => { sumMedian = sumMedian+histogramList[columnIndex+index+1][median]; });
