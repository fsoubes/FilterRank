...
const varianceFilter = (raster, graphContext, kernel, copy_mode = true) => {
  ...
  switch (raster.type) {
    ...
    const getFragmentSource = (samplerType,outVec,kernelLength,sum,sum2,total) => {
	...
	void main() {
	  ...
	  vec3 sum = vec3(${sum});
	  vec3 sum2 = vec3(${sum2});
	    outColor = vec(${total},1.0);
    }`;
  }

    let the_shader = gpu.createProgram(graphContext,src_vs,getFragmentSource(samplerType,outColor,kernel.length,sum,sum2,variance));
	    	
    // Step #1: Create - compile + link - shader progra

    //let the_shader = gpu.createProgram(graphContext,src_vs,getFragmentSource(samplerType,outColor, kernel.length, vectorType,uintType,variance));
