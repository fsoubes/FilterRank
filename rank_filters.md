...
The results of the benchmark speed's execution on the image 8 bit and RGB are presented on the figure 11 on four boxplots. On the top left we can see that the median filter has still the highest time exexcution mean of 32.16 ms. The maximum and minimun filter functions have respectivly a time's execution of 17.65 and 17.63 ms. At last the variance filter function does 16.18 ms. 
