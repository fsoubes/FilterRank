...
However for the 16 bit, we werent able to find a solution to find a threshold that's fit to display the image with variance filter probably due to the the texture gl.REDUI16
...
When I try to add the step implementation for the tresholding the execution time was between 2 and 3 times longer than without it. Hence, it was not cost effective to implement it.
