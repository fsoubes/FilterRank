...
By definition a rank filter is a filter that ranked, all the pixels within the window. The output of the filter is the pixel value selected from a specified position in this ranked list. For an identical window the pixel value will differ in function of the filters used (median, min, max and variance). Moreover the size of the window is also influencing the output pixel.
