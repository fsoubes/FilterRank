...
Afterwards a better functionnal method was proposed by Mr.Jean-Christophe TAVEAU using a reduce with the use of an accumulator in order to compute the summed-area table, this method is used in the implementation of the variance filter. The main advantages of this method is that it is 100% functionnal whereas the previous method even if faster was not totally functionnal because of the two forEach moreover it uses less characters than the other method (207 characters against 336 characters). However this method has also some disadvantages caused by the accumulator that cost as an extra row and forEach is way more faster than reduce. All these methods are still provided in the variance filter script.
