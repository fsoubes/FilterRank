...
### Filling edges 
The first part of the algorithm is the _remplissage_ function which allow to obtain a padded image around the edges. This function can work either for the row filter or columns filter with the paramater _type_. Depending of the type of the images and the filter filtre_ paramater we want to apply, this function will add extra values on the edges to help compute the min_max values of the image without interfering with the computations. The associated pseudo code is represented down below :
...
The same kind of operation applies for the line filter, the computation of the min or max function is then calculate with the _kernelsize_ function where an exampleof the pseudo code is down below :
...
![](images/Smith-Waterman-Algorithm-Scoring-2.png)
